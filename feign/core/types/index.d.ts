 ...
 *请求认证类型
 */
type RequestAuthenticationType = AuthenticationType | string | number;
...
type RequestProgressBarFunction<T extends ProgressBarOptions = ProgressBarOptions> = (progressBarOptions?: T) => CloseRequestProgressBarFunction;
type RequestProgressBar = RequestProgressBarInterface | RequestProgressBarFunction;
 ...
 * @param url 接口请求路径
 */
type ApiPermissionProbeStrategyFunction = (url: string) => Promise<MappingHeaders>;
/**
 * 用于判断接口请求路径是否需要权限
 */
 ...
 * {@see HeadMapping}
 */
type ApiPermissionProbeStrategy = ApiPermissionProbeStrategyInterface | ApiPermissionProbeStrategyFunction;
...
type FeignConfigurationConstructor<C extends BaseFeignClientConfiguration = BaseFeignClientConfiguration> = {
    new (...args: any[]): C;
};
 ...
 */
type ClientHttpRequestInterceptorFunction<T extends HttpRequest = HttpRequest> = (req: T) => Promise<T>;
/**
 ...
 * http request body
 */
type HttpRequestBody = string | Record<string, any>;
/**
 * http request client
...
type HttpResponseEventHandler = (request: FeignRequestOptions, response: HttpResponse) => void;
 ...
 */
declare const getRequestFeignConfiguration: (context: HttpRequestContext) => Readonly<FeignHttpConfiguration> | null;
/**
 ...
 * @param options
 */
declare const feignHttpFunctionBuilder: (options: FeignClientOptions<FeignHttpConfiguration>) => FeignHttpFunctionBuilder;
 ...
 */
type ResponseErrorHandlerFunction<T extends HttpRequest = HttpRequest, E = any> = (request: T, response: HttpResponse<any>) => Promise<E>;
...
declare class TraceRequestExecutorInterceptor<T extends FeignRequestOptions = FeignRequestOptions> implements FeignClientExecutorInterceptor<T> {
    ...
    postError: (options: T, response: HttpResponse) => Promise<never>;
    postHandle: <E>(options: T, response: E) => Promise<Awaited<E>>;
...
type FileUploadStrategyResult = FileUploadStrategyResultInterface | string;
/**
 ...
 * @return I  对应的接口实例
 */
declare const converterFunctionInterface: <T, I>(handle: T | I) => I;
...
type FeignClientBuilder<T extends FeignProxyClient = FeignProxyClient> = FeignClientBuilderFunction<T> | FeignClientBuilderInterface<T>;
...
export { AbstractHttpClient, AbstractLog4jLogger, AbstractRequestFileObjectEncoder, ApiPermissionProbeInterceptor, ApiPermissionProbeStrategy, ApiPermissionProbeStrategyFunction, ApiPermissionProbeStrategyInterface, ApiSignatureStrategy, AsyncClientRequestDataValidator, AuthenticationClientHttpRequestInterceptor, AuthenticationStrategy, AuthenticationToken, AuthenticationType, AutoFileUploadOptions, BaseFeignClientConfiguration, BaseRequestMappingOptions, BusinessResponseExtractorFunction, CacheCapableAuthenticationStrategy, ClientHttpRequestInterceptor, ClientHttpRequestInterceptorFunction, ClientHttpRequestInterceptorInterface, ClientRequestDataValidator, ClientRequestDataValidatorHolder, CodecFeignClientExecutorInterceptor, CommonResolveHttpResponse, ConsoleLogger, DataObfuscation, DateConverter, DateEncoder, DefaultFeignClientBuilder, DefaultHttpFeignClientExecutor as DefaultFeignClientExecutor, defaultFeignLo4jFactory as DefaultFeignLog4jFactory, DefaultHttpClient, DefaultNoneNetworkFailBack as DefaultNetworkStatusListener, DefaultUriTemplateHandler, DeleteMapping, Enum, FEIGN_CLINE_META_KEY, Feign, FeignClient, FeignClientBuilder, FeignClientBuilderFunction, FeignClientBuilderInterface, FeignClientExecutor, FeignClientExecutorInterceptor, FeignClientMethodConfig, FeignClientOptions, FeignClientType, FeignConfigurationConstructor, registry as FeignConfigurationRegistry, FeignHttpClientFunction, FeignHttpClientPromiseFunction, FeignHttpConfiguration, FeignLog4jFactory, FeignProxyClient, FeignRequestBaseOptions, FeignRequestContextOptions, FeignRequestOptions, FeignRetry, FileUpload, FileUploadProgressBar, FileUploadProgressBarOptions, FileUploadStrategy, FileUploadStrategyResult, FileUploadStrategyResultInterface, GenerateAnnotationMethodConfig, GetMapping, HttpAdapter, HttpClient, HttpErrorResponseEventPublisherExecutorInterceptor, HttpMediaType, HttpMethod, HttpRequest, HttpRequestBody, HttpRequestDataEncoder, HttpResponse, HttpResponseDataDecoder, HttpResponseEventHandler, HttpResponseEventHandlerSupplier, HttpResponseEventListener, HttpResponseEventPublisher, HttpRetryOptions, HttpStatus, Log4jLevel, Log4jLogger, MappedClientHttpRequestInterceptor, MappedFeignClientExecutorInterceptor, MappedInterceptor, Mapping, MappingHeaderType, MappingHeaders, NEVER_REFRESH_FLAG, NetworkClientHttpRequestInterceptor, NetworkFeignClientExecutorInterceptor, NetworkStatus, NetworkStatusListener, NetworkType, NoneNetworkFailBack, PatchMapping, PostMapping, ProcessBarExecutorInterceptor, ProgressBarOptions, PutMapping, QueryParamType, RequestAuthenticationType, RequestHeaderResolver, RequestMapping, RequestProgressBar, RequestTracer, RequestURLResolver, ResolveHttpResponse, ResponseErrorHandler, ResponseErrorHandlerFunction, ResponseErrorHandlerInterFace, ResponseExtractor, ResponseExtractorFunction, ResponseExtractorInterface, RestOperations, RestTemplate, RetryHttpClient, RoutingClientHttpRequestInterceptor, Signature, SimpleApiSignatureStrategy, SimpleHttpResponseEventListener, SimpleHttpResponseEventPublisher, SimpleNoneNetworkFailBack as SimpleNetworkStatusListener, SmartHttpResponseEventListener, TraceRequestExecutorInterceptor, UIOptions, UNAUTHORIZED_RESPONSE, UriTemplateHandler, UriTemplateHandlerFunction, UriTemplateHandlerInterface, UriVariable, ValidateInvokeOptions, ValidatorDescriptor, contentLengthName, contentTransferEncodingName, contentTypeName, converterFunctionInterface, defaultApiModuleName, defaultFeignClientBuilder, defaultUriTemplateFunctionHandler, feignHttpFunctionBuilder, filterNoneValueAndNewObject, generateFeignClientAnnotation, generateMapping, getRequestFeignClientMethodConfiguration, getRequestFeignConfiguration, grabUrlPathVariable, headResponseExtractor, matchMediaType, objectResponseExtractor, optionsMethodResponseExtractor, queryStringify, registerAnnotationMetadata, responseIsFile, responseIsJson, responseIsText, restfulRequestURLResolver, serializeRequestBody, setDefaultFeignLo4jFactory, simpleRequestHeaderResolver, simpleRequestURLResolver, stringDateConverter, supportRequestBody, timeStampDateConverter, voidResponseExtractor };
